...
#Execução
def execution():
    #subprocess.run(['sudo', 'usermod', '-aG', 'pulse-access', 'carlos_'])
    subprocess.run(['pulseaudio', '--start'])
    set_volume((vol))

    #diretoriao atual + subpasta + arquivo
    file =os.path.join(os.path.dirname(os.path.abspath(__file__)), 'audio', audios[audio])
...
if args.i:
    ...
    while True:
        ...
        try:
            ...
            print(f'Ocorreu uma exceção não esperada: {e}')
        ...
        #print(audio)
            audio = int(audio)
            vol = float(vol)
            conexao.send((f"{Fore.GREEN}[*] Executando a audio...{Style.RESET_ALL}").encode('utf-8'))
            execution()
        else:
            conexao.close()
            conexao, endereco_cliente = mysocket.accept()
